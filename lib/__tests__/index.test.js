...
describe('waypoint-haikus index', () => {
  it('should return a resolved promise when there are new tweets', () =>
    expect(waypointHaikus()).resolves.toEqual({
  ...
  it('should return a resolved promise when there are no new tweets', () => {
    ...
    return expect(waypointHaikus()).resolves.toEqual({
  ...
  it('should return a resolved promise when a haiku tweet is found', () => {
    ...
    return expect(waypointHaikus()).resolves.toEqual({
      success: { newLastSeenTweet: '3', prevLastSeenTweet: undefined, tweets: ['https://twitter.com/WaypointHaikus/status/4'] },
    });
  });
});
