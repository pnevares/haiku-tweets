...
describe('get-last-seen-tweet', () => {
  it('should resolve with a Promise', () => {
    ...
    return expect(getLastSeenTweet({ keys: 'keys', s3: 's3' })).resolves.toEqual('hello');
  });

  it('should absorb "missing object" errors', () => {
    AWS.__throwS3GetObject({ statusCode: 404 });
    return expect(getLastSeenTweet({ keys: 'keys', s3: 's3' })).resolves.toEqual(undefined);
  });

  it('should reject with any other error', () => {
    AWS.__throwS3GetObject('any other error');
    return expect(getLastSeenTweet({ keys: 'keys', s3: 's3' })).rejects.toThrow('any other error');
  });
});
