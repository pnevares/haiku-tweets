...
describe('filter-timeline', () => {
  ...
  it('returns compliant tweets', () =>
    expect(filterTimeline('bot', tweets)).resolves.toHaveLength(2));

  it('removes retweets from a timeline', () => {
    tweets.data[0].retweeted_status = true;

    return expect(filterTimeline('bot', tweets)).resolves.toHaveLength(1);
  });

  it('removes the bot account\'s tweets from the timeline', () => {
    tweets.data[0].user.screen_name = 'bot';

    return expect(filterTimeline('bot', tweets)).resolves.toHaveLength(1);
  });

  it('simplifies full_text', () =>
    expect(filterTimeline('bot', tweets))
  ...
  it('removes urls from full_text', () => {
    ...
    return expect(filterTimeline('bot', tweets)).resolves.toEqual(expect.arrayContaining([
      expect.objectContaining({ simple_text: 'just a tweet' }),
    ]));
  });
  ...
  it('can handle an empty array of tweets', () => {
    ...
    return expect(filterTimeline('bot', tweets)).resolves.toHaveLength(0);
  });

  it('rejects on a badly-formatted tweet', () => {
    delete tweets.data[0].full_text;

    return expect(filterTimeline('bot', tweets)).rejects.toThrow();
  });
});
