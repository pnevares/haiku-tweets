...
describe('filter-timeline', () => {
  ...
  it('returns compliant tweets', () =>
    expect(filterTimeline('account', tweets)).resolves.toHaveLength(2));
  ...
  it('creates simple_text based on full_text', () =>
    expect(filterTimeline('account', tweets))
  ...
  describe('can handle real-world tweets', () => {
    it('with a strange apostrophe', () => {
      ...
      return expect(filterTimeline('account', tweets)).resolves.toEqual(expect.arrayContaining([
        expect.objectContaining({ simple_text: 'the BB in BloodBorne stands for Big Boy sorry i dont make the rules' }),
      ]));
    });
  });
});
