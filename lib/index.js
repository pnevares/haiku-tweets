...
module.exports = () => {
  ...
  return getConfig()
    ...
    .then((results) => {
      const tweets = results.map((c) => c.message // rejected promise
        || c.data.errors // twitterApi error response
        || `https://twitter.com/${config.twitter.account}/status/${c.data.id_str}`);
